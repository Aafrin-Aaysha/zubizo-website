...
export default function MaterialsPage() {
    ...
    const fetchMaterials = async () => {
        ...
        try {
            ...
            // Map legacy categories to new structure seamlessly for UI
            const mappedData = data.map((m: any) => {
                let mappedCategory = m.category;
                if (m.category === 'Envelope Shape') mappedCategory = 'Envelopes';
                else if (m.category === 'Envelope Color') mappedCategory = 'Envelopes';
                else if (m.category === 'Wax Seal') mappedCategory = 'Wax Seals';
                else if (m.category === 'Ribbon') mappedCategory = 'Ribbons';
                else if (m.category === 'Paper Type') mappedCategory = 'Paper Types';
                
                return { ...m, category: mappedCategory, _legacyCategory: m.category };
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            name,
            category, // We pass the mapped primary category to DB now
            imageUrl,
        ...
        <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500">Manage {selectedParent ? `${selectedParent.name} variants` : 'your design materials hierarchically'}.</p>
                </div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    {PRIMARY_CATEGORIES.map(cat => (
                        <button
                            ...
                            className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeCategory === cat ? 'bg-charcoal text-white shadow-xl' : 'bg-white text-gray-400 hover:bg-gray-50'}`}
                        >
                            {cat}
                        </button>
                    ))}
            ...
            {isLoading ? (
                ...
                <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
                    ...
                    <p className="text-slate-500 mb-6 text-sm">You haven't added any {selectedParent ? `variants for ${selectedParent.name}` : activeCategory} yet.</p>
                    <button 
                        onClick={() => openModal(null, selectedParent ? selectedParent._id : '')}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {filteredItems.map(material => {
                        ...
                        return (
                            <div key={material._id} className="group relative bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all hover:-translate-y-1 flex flex-col">
                                {/* Hover Actions */}
                                <div className="absolute inset-0 bg-black/50 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 z-10 p-4">
                                    {!selectedParent && (
                                        <button 
                                            ...
                                            className="w-full bg-white text-charcoal font-black text-[10px] uppercase tracking-widest py-3 rounded-xl hover:bg-gray-100 transition-colors flex items-center justify-center gap-2"
                                        >
                                    ...
                                    <div className="flex gap-2 w-full">
                                        <button 
                                            ...
                                            className="flex-1 bg-white text-lavender font-bold py-3 rounded-xl hover:bg-gray-100 transition-colors flex justify-center"
                                        >
                                            <Edit2 size={16} />
                                        </button>
                                        <button 
                                            onClick={() => handleDelete(material._id)}
                                            className="flex-1 bg-white text-red-500 font-bold py-3 rounded-xl hover:bg-gray-100 transition-colors flex justify-center"
                                        >
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                ...
                                {/* Content */}
                                <div className="aspect-square relative overflow-hidden bg-slate-50 flex-shrink-0 border-b border-gray-50">
                                    {material.imageUrl ? (
                                ...
                                <div className="p-4 text-center flex-1 flex flex-col justify-center">
                                    <h3 className="font-bold text-slate-800 text-sm line-clamp-1">{material.name}</h3>
                                    {!selectedParent && (
                                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">{childrenCount} Variants</p>
                                    )}
                                </div>
                            </div>
                        );
            ...
            <AnimatePresence>
                {isModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            <div className="p-6 md:p-8 border-b border-gray-50 flex justify-between items-center bg-gray-50/50 flex-shrink-0">
                                <div>
                                    <h2 className="text-2xl font-serif text-[#6E4B8B]">{editingMaterial ? 'Update Material' : 'Add Material'}</h2>
                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">
                                        {selectedParent ? `Variant for ${selectedParent.name}` : activeCategory}
                                    </p>
                                </div>
                                <button type="button" onClick={() => setIsModalOpen(false)} className="w-10 h-10 hover:bg-white rounded-xl flex items-center justify-center transition-colors shadow-sm border border-gray-100"><X size={20} /></button>
                            </div>
                            ...
                            <div className="p-6 md:p-8 overflow-y-auto">
                                <form id="material-form" onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <select
                                                ...
                                                className="w-full px-4 py-3 bg-gray-50 border border-transparent rounded-xl focus:bg-white focus:border-lavender outline-none transition-all font-bold text-sm text-gray-400"
                                    ...
                                    {/* Parent Linkage */}
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-charcoal uppercase tracking-widest flex items-center justify-between">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex-1 relative">
                                                ...
                                                />
                                                <div className={`w-full px-4 py-6 border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2 transition-all ${imageUrl ? 'border-gray-200 bg-gray-50' : 'border-lavender/30 bg-lavender/5 hover:bg-lavender/10'}`}>
                            ...
                            <div className="p-6 md:p-8 border-t border-gray-50 flex items-center justify-end gap-3 bg-white flex-shrink-0">
