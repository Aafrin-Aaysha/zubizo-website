                    ...
                    <form onSubmit={handleUpdate} className="space-y-5">
                        ...
                        <section className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                            ...
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-[9px] font-black text-charcoal uppercase tracking-widest block">Current Password</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-10 pr-10 py-2.5 bg-gray-50 border border-transparent rounded-xl focus:bg-white focus:border-lavender outline-none transition-all font-bold text-charcoal"
                                            placeholder="••••••••"
                                        />
                                        <button
                                            type="button" onClick={() => setShowPassword(!showPassword)}
                                            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-lavender transition-colors"
                                        >
                                            {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                                        </button>
                                    </div>
                                    <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Required only if changing password</p>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <label className="text-[9px] font-black text-charcoal uppercase tracking-widest block">New Password</label>
                                        <div className="relative">
                                            <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300" size={16} />
                                            <input
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-[9px] font-black text-charcoal uppercase tracking-widest block">Confirm New Password</label>
                                        <div className="relative">
                                            <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300" size={16} />
                                            <input
                                                type={showNewPassword ? "text" : "password"}
                        ...
                        {adminRole === 'super-admin' && (
                            <section className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                                <h3 className="text-[10px] font-black text-gray-405 uppercase tracking-widest flex items-center gap-2">
                                    <ShieldCheck size={12} /> System Permissions
                                </h3>
                                
                                <div className="flex items-center justify-between">
                                    <div className="space-y-0.5">
                                        <h4 className="font-bold text-charcoal text-xs">Global System View</h4>
                                    ...
                                    <button
                                        ...
                                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${formData.showGlobalData ? 'bg-charcoal' : 'bg-gray-200'}`}
                                    >
                                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${formData.showGlobalData ? 'translate-x-6' : 'translate-x-1'}`} />
                                    </button>
                                </div>
                            </section>
                        )}
