...
export default function DigitalInvitesAdminPage() {
    ...
    const [activeTab, setActiveTab] = useState<'E-Invite' | 'E-Website'>('E-Invite');
    ...
    const fetchInitialData = React.useCallback(async () => {
        ...
        try {
            ...
            if (digitalCats.length > 0 && !formData.categoryId) {
                setFormData(prev => ({ ...prev, categoryId: digitalCats[0]._id }));
            }
        } catch (error) {
    ...
    const filteredDesigns = designs.filter(design => {
        ...
        const matchesTab = activeTab === 'E-Invite' 
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validate Demo URL for Websites
        ...
        // Ensure at least one package (can be a hidden/default one for digital)
        const finalData = { ...formData };
        if (finalData.packages.length === 0) {
            finalData.packages = [{
                title: activeTab === 'E-Website' ? 'Website Access' : 'Digital Delivery',
                inclusions: [activeTab === 'E-Website' ? 'Interactive Link' : 'JPEG/PNG/PDF Bundle'],
    ...
    const openModal = (design: any = null) => {
        if (design) {
            ...
            const targetCat = categories.find(c => 
                activeTab === 'E-Invite' ? c.name === 'Digital E-Invite' : c.name === 'Premium E-Website'
            );
            setFormData({
                sku: '',
                name: '',
        ...
        <div className="space-y-8 pb-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-gray-500 mt-1 font-medium">Manage E-Invites and Interactive Websites.</p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    >
                        <Plus size={20} />
                        New {activeTab === 'E-Invite' ? 'E-Invite' : 'E-Website'}
                    </button>
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
                <div className="bg-gray-100 p-1.5 rounded-2xl flex items-center gap-1 w-full md:w-auto">
                    <button
                        onClick={() => setActiveTab('E-Invite')}
                        className={cn(
                            "flex-1 md:w-40 px-6 py-3 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-all",
                            activeTab === 'E-Invite' ? "bg-white text-lavender shadow-sm" : "text-gray-400 hover:text-gray-600"
                        )}
                    >
                        <ImageIcon size={16} />
                        E-Invites
                    ...
                    <button
                        onClick={() => setActiveTab('E-Website')}
                        className={cn(
                            "flex-1 md:w-40 px-6 py-3 rounded-xl flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-all",
                            activeTab === 'E-Website' ? "bg-white text-lavender shadow-sm" : "text-gray-400 hover:text-gray-600"
                        )}
                    >
                        <Globe size={16} />
                        E-Websites
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 text-gray-400 text-[10px] font-black uppercase tracking-widest border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-8 py-5">{activeTab === 'E-Invite' ? 'Files' : 'Demo Link'}</th>
                                ...
                                <tr>
                                    <td colSpan={5} className="px-8 py-32 text-center">
                                        <div className="flex flex-col items-center justify-center gap-4">
                                            ...
                                            <p className="font-bold text-gray-900 text-lg">No {activeTab === 'E-Invite' ? 'E-Invites' : 'E-Websites'} found</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={design._id} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="px-8 py-5">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Pricing: ₹{getStartingPrice(design)}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-8 py-5">
                                            {activeTab === 'E-Website' ? (
                                                ...
                                                <span className="text-xs font-medium text-gray-500">{design.images?.length || 0} Assets</span>
                                            )}
                                        </td>
                                        <td className="px-8 py-5">
                                            <div className={cn(
                ...
                {isModalOpen && (
                    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 md:p-6 overflow-hidden">
                        ...
                        <motion.div
                            ...
                            className="relative w-full max-w-4xl bg-gray-50 max-h-[90vh] shadow-2xl rounded-[2.5rem] flex flex-col overflow-hidden"
                        >
                            <div className="p-8 bg-white border-b border-gray-100 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-2xl bg-lavender/10 text-lavender flex items-center justify-center">
                                        {activeTab === 'E-Invite' ? <ImageIcon size={24} /> : <Globe size={24} />}
                                    </div>
                                    <div>
                                        <h2 className="text-2xl font-black text-charcoal">{editingDesign ? 'Update' : 'Create'} {activeTab === 'E-Invite' ? 'E-Invite' : 'E-Website'}</h2>
                                        <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Dedicated Digital Configuration</p>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div className="space-y-8">
                                        <section className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-4">
                                            ...
                                            <div className="space-y-4">
                                                <div>
                                                    <label className="text-[10px] font-black text-charcoal uppercase tracking-widest mb-1.5 block">Invite Name</label>
                                                    <input type="text" required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-lavender outline-none transition-all font-bold text-charcoal" />
                                                </div>
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        <label className="text-[10px] font-black text-charcoal uppercase tracking-widest mb-1.5 block">SKU</label>
                                                        <input type="text" required value={formData.sku} onChange={e => setFormData({ ...formData, sku: e.target.value })} className="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-lavender outline-none transition-all font-black" />
                                                    </div>
                                                    <div>
                                                        <label className="text-[10px] font-black text-charcoal uppercase tracking-widest mb-1.5 block">Price (₹)</label>
                                                        <input type="number" required value={formData.basePrice} onChange={e => setFormData({ ...formData, basePrice: parseFloat(e.target.value) || 0 })} className="w-full px-5 py-3.5 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:border-lavender font-black" />
                                                    </div>
                                                </div>
                                                {activeTab === 'E-Website' && (
                                                    <div>
                                                        <label className="text-[10px] font-black text-charcoal uppercase tracking-widest mb-1.5 block">Live Website URL</label>
                                                        <input type="url" required value={formData.demoUrl} onChange={e => setFormData({ ...formData, demoUrl: e.target.value })} className="w-full px-5 py-3.5 bg-lavender/5 border-2 border-lavender/20 rounded-2xl focus:bg-white focus:border-lavender outline-none transition-all font-bold text-lavender" placeholder="https://demo.zubizo.com/..." />
                                                    </div>
                                                )}
                                                <div>
                                                    <label className="text-[10px] font-black text-charcoal uppercase tracking-widest mb-1.5 block">Status</label>
                                                    <button type="button" onClick={() => setFormData({ ...formData, isActive: !formData.isActive })} className={cn("w-full px-5 py-3.5 rounded-2xl border-2 flex items-center justify-between transition-all", formData.isActive ? "border-green-100 bg-green-50/50 text-green-700" : "border-gray-100 bg-gray-50 text-gray-400")}>
                                                        <span className="font-bold text-sm">{formData.isActive ? 'Active & Visible' : 'Hidden from Catalog'}</span>
                                                        <div className={cn("w-1.5 h-1.5 rounded-full shrink-0", formData.isActive ? "bg-green-500 animate-pulse" : "bg-gray-300")} />
                                                    </button>
                                                </div>
                                            </div>
                                        </section>
                                    ...
                                    <div className="space-y-8">
                                        <section className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-6">
                                            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                                <Upload size={14} /> {activeTab === 'E-Invite' ? 'Portfolio Images' : 'Website Thumbnails'}
                                            </h3>
                                            <div className="grid grid-cols-2 gap-4">
                                                {formData.images.map((img, idx) => (
                                                    <div key={idx} className="aspect-[3/4] rounded-2xl overflow-hidden relative group border border-gray-100">
                                                        <img src={img} alt="" className="w-full h-full object-cover" />
                                                        <button type="button" onClick={() => setFormData({ ...formData, images: formData.images.filter((_, i) => i !== idx) })} className="absolute top-2 right-2 w-7 h-7 bg-red-500 text-white rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={14} /></button>
                                                    </div>
                                                ))}
                                                <label className="aspect-[3/4] rounded-2xl border-2 border-dashed border-gray-100 flex flex-col items-center justify-center gap-3 text-gray-300 hover:border-lavender hover:text-lavender transition-all cursor-pointer bg-gray-50/50">
                                                    ...
                                                    <Upload size={24} />
                                                    <span className="text-[8px] font-black uppercase text-center px-2">Upload Files</span>
                                                </label>
                                            </div>
                                        </section>
                                    </div>
                            ...
                            <div className="p-8 bg-white border-t border-gray-100 flex justify-between items-center shrink-0">
                                <button onClick={closeModal} className="px-8 py-4 font-black text-xs uppercase tracking-widest text-gray-400 hover:text-charcoal transition-colors">Discard</button>
                                <button type="submit" form="digital-form" disabled={isSubmitting} className="px-10 py-4 bg-charcoal hover:bg-black text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-charcoal/20 transition-all flex items-center justify-center gap-2">
                                    {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : (editingDesign ? 'Save Changes' : `Publish ${activeTab}`)}
                                </button>
                            </div>
                        </motion.div>
                    </div>
